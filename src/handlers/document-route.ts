...
import {CreatFolderValidation, ListDocumentRequest, UploadFileToFolderValidation, createDocumentValidation, idDocumentValidation, listDocumentValidation, moveFileValidation} from "./validator/document-validator";
...
export const documentsRoutes = (app: express.Express) => {
    ...
    * */
    ...
    app.post("/create-folder/:userId",(req:Request,res:Response)=>{
        try{
            ...
            const result = folder.createFolder(creatfolder.value.name,creatfolder.value.userId)
            res.status(200).send(result)
        }catch(error){
    ...
    app.post("/folsders/:folderid/upload/:userId/files",upload.single('file'),(req:Request,res:Response)=>{
        try{
            ...
            const newfile = new DocumentUseCase(AppDataSource)
            const result = newfile.uploadToFolder(folderId, req.file, UploadFileToFolder.value.userId)
 ...
 // app.post("/documents", upload.single('file'), async (req: Request, res: Response) => {
    ...
    //         // vérification si la requête contient un fichier
    //         if (!req.file) {
    //             res.status(400).send({ error: "File is required" });
    //             return;
    //         }
